...
class guestregistration(models.Model):
    ...
    #guestname <- related field found as a computed field called name 
    #in the model hotel.guests
    ...
    def action_reserve(self):
        for rec in self:
            if not(rec.guest_id):  
                ...
                raise ValidationError('Please supply a valid check in date.')
            elif not(rec.datetoSched):    
                raise ValidationError('Please supply a valid check out date.')
